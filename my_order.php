		...
		<section id="content_section">
			<div class="my_order_page gerenric_padding">
				<div class="page_width_1480">
					<h1>My Orders</h1>
					<?php
					$Query = "SELECT oi.*, ord.user_id, ord.ord_datetime, ord.ord_udate, di.dinfo_fname, di.dinfo_phone, di.dinfo_address, pro.pro_description_short, pro.pro_udx_seo_internetbezeichung, pg.pg_mime_source FROM order_items AS oi LEFT OUTER JOIN orders AS ord ON ord.ord_id = oi.ord_id LEFT OUTER JOIN delivery_info AS di ON di.ord_id = oi.ord_id LEFT OUTER JOIN products AS pro ON pro.supplier_id = oi.supplier_id LEFT OUTER JOIN products_gallery AS pg ON pg.supplier_id = oi.supplier_id AND pg.pg_mime_purpose = 'normal' AND pg.pg_mime_order = '1' WHERE ord.user_id = '" . $_SESSION['UID'] . "' ORDER BY ord.ord_datetime DESC";
										...
										<div class="place_div">
											<div class="placeser_name"> <?php print($row->dinfo_fname); ?> <i class="fa fa-caret-down"></i>
												<div class="placeser_info">
													<ul>
														<li>
															<div class="placeser_title">Phone:</div>
															<div class="placeser_text"><?php print($row->dinfo_phone); ?></div>
														</li>
														<li>
															<div class="placeser_title">Location:</div>
															<div class="placeser_text"><?php print($row->dinfo_address); ?></div>
														</li>
													</ul>
												</div>
											</div>
