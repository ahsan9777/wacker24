...
if (isset($_REQUEST['btn_login'])) {
	...
	$usernameError = null;
	$passwordError = null;
	$password = md5(dbStr(trim($_REQUEST['user_password'])));
	...
	if ($valid) {
		...
		$rs = mysqli_query($GLOBALS['conn'], "SELECT * FROM users WHERE  user_password='" . $password . "' AND user_name='" . $username . "' AND status_id = '1'") or die(mysqli_error($GLOBALS['conn']));
					...
					<div class="login_box">
						...
						<form class="gerenric_form" name="frm" id="frm" method="post" action="<?php print($_SERVER['PHP_SELF'] . "?" . $_SERVER['QUERY_STRING']); ?>" role="form" enctype="multipart/form-data">
							<ul>
								...
								<li><div class="forgot_password"><a href="javascript:void(0)">Forgot your password?</a></div></li>
								<li><div class="form_term">By registering, you agree to our <a href="javascript:void(0)">Terms and Conditions</a>.Please read our <a href="javascript:void(0)">Privacy Policy </a>, our <a href="javascript:void(0)">Cookie Notice and our Interest-Based Advertising Notice .</a></div></li>
							</ul>
						</form>
					</div>
					<div class="or_div"><div class="or_div_inner">New to Wacker24?</div></div>
					<div class="new_account_btn"><a href="registration.php"><div class="gerenric_btn">Create your Wacker24 account here</div></a></div>
				</div>
			</div>
		</section>
		<!--LOGIN_PAGE_END-->
