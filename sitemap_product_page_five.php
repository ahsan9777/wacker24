...
<urlset xmlns="http://www.sitemaps.org/schemas/sitemap/0.9">
    ...
    <?php
    $rsp = mysqli_query($GLOBALS['conn'], "SELECT pro_cdate, pro_udate, supplier_id FROM products WHERE pro_status='1' AND (pro_id >= 32000 AND pro_id < 40000) ");
    if (mysqli_num_rows($rsp) > 0) {
        while ($rwp = mysqli_fetch_object($rsp)) {
            ...
            //$date->format('c');
            $pgURL = $GLOBALS['siteURL'] .product_detail_url($rwp->supplier_id);
            echo "<url>
                    <loc>" . $pgURL . "</loc>
