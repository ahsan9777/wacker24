...
function cart_to_order($user_id, $usa_id, $pm_id, $entityId = null, $ord_payment_transaction_id = null)
{
	...
	if (mysqli_num_rows($rs1) > 0) {
		...
		mysqli_query($GLOBALS['conn'], "INSERT INTO orders (ord_id, user_id, user_visit, guest_id, ord_gross_total, ord_gst, ord_discount, ord_amount, ord_shipping_charges, ord_payment_entity_id, ord_payment_transaction_id, ord_payment_method, ord_note, ord_datetime) VALUES ('" . $ord_id . "', '" . $user_id . "', '".$_SESSION['user_visit']."', '" . $_SESSION['sess_id'] . "', '" . $row1->cart_gross_total . "',  '" . $row1->cart_gst . "',  '" . $row1->cart_discount . "', '" . $row1->cart_amount . "', '" . $ord_shipping_charges . "', '" . $entityId . "', '" . $ord_payment_transaction_id . "', '" . $pm_id . "', '" . $ord_note . "', '" . date_time . "')") or die(mysqli_error($GLOBALS['conn']));
	...
	if ($orders_table_check == 1 && $order_items_table_check == 1) {
		if ($pm_id == 7) {
			...
			$mailer = new Mailer();
			$mailer->vorkasse($dinfo_email, $ord_id, $dinfo_fname . ' ' . $dinfo_lname, $order_net_amount);
		}
