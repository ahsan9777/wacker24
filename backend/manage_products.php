...
if (isset($_REQUEST['btnImport']) || isset($_REQUEST['btnImportSchulranzen'])) {
    ...
    if (isset($_REQUEST['btnImportSchulranzen'])) {
        ...
        mysqli_query($GLOBALS['conn'], "UPDATE products SET  pro_status = '0' WHERE pro_custom_add = '0' AND pro_type = '" . $pro_type . "'") or die(mysqli_error($GLOBALS['conn']));
    }
    /*print('<pre>');
    ...
    foreach ($xml->T_NEW_CATALOG->PRODUCT as $rl) {
        ...
        $pro_udx_seo_epag_title = isset($rl->USER_DEFINED_EXTENSIONS->{'UDX.SOE.EPAG_TITLE'}) ? $rl->USER_DEFINED_EXTENSIONS->{'UDX.SOE.EPAG_TITLE'} : '';
        ...
        if (mysqli_num_rows($rs1) > 0) {
            ...
            mysqli_query($GLOBALS['conn'], "INSERT INTO manufacture (manf_id, manf_name, manf_name_params) VALUES ('" . $manf_id . "', '" . dbStr(trim($pro_manufacture_name)) . "', '" . dbStr(url_clean(convertGermanChars(trim($pro_manufacture_name)))) . "')") or die(mysqli_error($GLOBALS['conn']));
        }
        ...
        if ($pro_type == 20) {
            ...
            if (mysqli_num_rows($rs3) > 0) {
                ...
                mysqli_query($GLOBALS['conn'], "INSERT INTO category (cat_id, group_id, parent_id, cat_title_de, cat_params_de) VALUES ('" . $cat_id . "', '" . $group_id . "', '20', '" . dbStr(trim($pro_udx_seo_pk)) . "', '" . dbStr(url_clean(convertGermanChars(trim($pro_udx_seo_pk)))) . "')") or die(mysqli_error($GLOBALS['conn']));
            }
        }
        ...
        if (!empty($pro_feature)) {
            ...
            for ($i = 0; $i < count($pro_feature); $i++) {
                if ($i > 0) {
                    ...
                    $pf_fname_params_de = dbStr(url_clean(convertGermanChars(trim($pro_feature[$i]->FNAME))));
                    $pf_fvalue = $pro_feature[$i]->FVALUE;
                    $pf_fvalue_params_de = dbStr(url_clean(convertGermanChars(trim($pro_feature[$i]->FVALUE))));
                                ...
                                <tbody>
                                    ...
                                    ?>
                                            <tr>
                                                ...
                                                <td>
                                                    <button type="button" class="btn btn-xs btn-success btn-style-light w-auto" target="_blank" title="View" onClick="javascript: window.open ('<?php print($GLOBALS['siteURL'].$row->pro_udx_seo_internetbezeichung_params_de); ?>');"><span class="material-icons icon material-xs">visibility</span></button>
