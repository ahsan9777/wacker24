...
if (isset($_REQUEST['btnActive'])) {
    if (isset($_REQUEST['chkstatus'])) {
        for ($i = 0; $i < count($_REQUEST['chkstatus']); $i++) {
            mysqli_query($GLOBALS['conn'], "UPDATE appointment_holidays SET ah_status='1' WHERE ah_id = " . $_REQUEST['chkstatus'][$i]);
        }
...
if (isset($_REQUEST['btnInactive'])) {
    if (isset($_REQUEST['chkstatus'])) {
        for ($i = 0; $i < count($_REQUEST['chkstatus']); $i++) {
            mysqli_query($GLOBALS['conn'], "UPDATE appointment_holidays SET ah_status='0' WHERE ah_id = " . $_REQUEST['chkstatus'][$i]);
        }
        $class = "alert alert-success";
        $strMSG = "Record(s) updated successfully";
    } else {
...
if (isset($_REQUEST['btnDelete'])) {
    if (isset($_REQUEST['chkstatus'])) {
        for ($i = 0; $i < count($_REQUEST['chkstatus']); $i++) {
            ...
            mysqli_query($GLOBALS['conn'], "DELETE FROM appointment_holidays WHERE ah_id = " . $_REQUEST['chkstatus'][$i]) or die(mysqli_error($_REQUEST['conn']));
        }
        $class = "alert alert-success";
            ...
            <section class="content" id="main-content">
                        ...
                        <form name="frm" id="frm" method="post" action="<?php print($_SERVER['PHP_SELF'] . "?" . $_SERVER['QUERY_STRING']); ?>" role="form" enctype="multipart/form-data">
                            <div class="row">
                                <div class="col-md-6 col-12 mt-3">
                                    ...
                                    <input type="date" class="input_style" name="ah_date" id="ah_date" value="<?php print($ah_date); ?>" placeholder="Title EN">
                                </div>
                ...
                <?php } else { ?>
                    <div class="table-controls">
                        <h1 class="text-white">Holidays Management</h1>
                    ...
                    <div class="main_table_container">
                        <form class="table_responsive" name="frm" id="frm" method="post" action="<?php print($_SERVER['PHP_SELF'] . "?" . $_SERVER['QUERY_STRING']); ?>" role="form" enctype="multipart/form-data">
                            <table>
                                ...
                                <tbody>
                                    <?php
                                    $Query = "SELECT * FROM appointment_holidays ORDER BY ah_date ASC";
                                    //print($Query);
                                    $counter = 0;
                                    $limit = 25;
                                    ...
                                    ?>
                                            <tr>
                                                <td><input type="checkbox" name="chkstatus[]" value="<?php print($row->ah_id); ?>"></td>
                                                <td><?php print(date('D F j, Y', strtotime($row->ah_date))); ?></td>
                                                <td>
                                                    <?php
                                                    if ($row->ah_status == 0) {
                                                ...
                                                <td>
                                                    <button type="button" class="btn btn-xs btn-primary btn-style-light w-auto" title="Edit" onClick="javascript: window.location = '<?php print($_SERVER['PHP_SELF'] . "?action=2&" . $qryStrURL . "ah_id=" . $row->ah_id); ?>';"><span class="material-icons icon material-xs">edit</span></button>
                                                </td>
                                            </tr>
                                    <?php
                                        }
