...
if (isset($_REQUEST['rr_status'])) {
    mysqli_query($GLOBALS['conn'], "UPDATE return_request SET rr_status = '".$_REQUEST['rr_status']."' WHERE rr_id = '" . $_REQUEST['rr_id'] . "'") or die(mysqli_error($GLOBALS['conn']));
    header("Location: " . $_SERVER['PHP_SELF'] . "?rr_id=" . $_REQUEST['rr_id'] . "&op=2");
} elseif (isset($_REQUEST['btnDelete'])) {
    mysqli_query($GLOBALS['conn'], "DELETE FROM `return_request` WHERE rr_id = '" . $_REQUEST['rr_id'] . "'") or die(mysqli_error($GLOBALS['conn']));
                ...
                <div class="container text-start">
                    ...
                    <div class="row mt-3 position-relative">
                        <div class="col-md-3 col-12 bg-secondary rounded-3">
                            <?php
                            $Query1 = "SELECT * FROM return_request ORDER BY rr_cdate DESC";
                            $rs1 = mysqli_query($GLOBALS['conn'], $Query1);
                            if (mysqli_num_rows($rs1)) {
                                while ($row1 = mysqli_fetch_object($rs1)) {
                            ?>
                                    <a href="<?php print($_SERVER['PHP_SELF'] . "?rr_id=" . $row1->rr_id); ?>" class="tab_container p-3 border-bottom">
                                        ...
                                        <div class="tab_detail">
                                            ...
                                            <div class="contact_date">
                                                <?php print(date('F j, Y H:i', strtotime($row1->rr_cdate))); ?>
                                            </div>
                                        </div>
                                    </a>
                            <?php
                        ...
                        <div class="col-md-9 col-12 bg-dark rounded-3 p-3">
                            <?php
                            $searchQuery = "ORDER BY rr_id DESC LIMIT 0,1";
                            if (isset($_REQUEST['rr_id']) && $_REQUEST['rr_id'] > 0) {
                                $searchQuery = "WHERE rr_id = '" . $_REQUEST['rr_id'] . "'";
                            }
                            $Query2 = "SELECT * FROM return_request " . $searchQuery . "";
